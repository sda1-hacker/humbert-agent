...
type CopyFileFactory struct{ maxBytes int64 }

func NewCopyFileFactory(maxBytes int64) (*CopyFileFactory, error) {
	if maxBytes <= 0 {
		return nil, errors.New("copy_file maxBytes 必须大于 0")
	}
	return &CopyFileFactory{maxBytes: maxBytes}, nil
}
...
func (f *CopyFileFactory) Build(ctx context.Context, scope humberttools.Scope) (einotool.InvokableTool, error) {
	return utils.InferTool(copyFileToolName, "Copy one regular file between paths allowed by the Agent Sandbox. The destination must be writable.", func(callCtx context.Context, input *FileCopyMoveInput) (*FileOperationOutput, error) {
		return copyFile(callCtx, scope, input, f.maxBytes, false)
	})
}
...
func (f *MoveFileFactory) Build(ctx context.Context, scope humberttools.Scope) (einotool.InvokableTool, error) {
	return utils.InferTool(moveFileToolName, "Move one regular file. Both the source and destination must have FULL path access under the Agent Sandbox.", func(callCtx context.Context, input *FileCopyMoveInput) (*FileOperationOutput, error) {
		return copyFile(callCtx, scope, input, f.maxBytes, true)
	})
}

func copyFile(ctx context.Context, scope humberttools.Scope, input *FileCopyMoveInput, maxBytes int64, move bool) (*FileOperationOutput, error) {
	if input == nil {
	...
	var err error
	if move {
		src, err = openSandboxTarget(ctx, scope, input.Source, sandbox.OpMoveSource)
	} else {
		src, err = openSandboxTarget(ctx, scope, input.Source, sandbox.OpRead)
	}
	if err != nil {
	...
	if sameSandboxPath(src.absolute, dst.absolute) {
		return nil, errors.New("source 与 destination 不能指向同一个文件")
	}
	in, err := src.root.Open(src.relative)
	if err != nil {
		return nil, fmt.Errorf("打开 source 失败: %w", err)
	}
	defer in.Close()
	info, err := in.Stat()
	if err != nil {
	...
	if int64(len(data)) > maxBytes {
		return nil, fmt.Errorf("文件超过限制 %d", maxBytes)
	}
	...
	return &FileOperationOutput{Source: src.display, Destination: dst.display, Bytes: int64(len(data))}, nil
}
